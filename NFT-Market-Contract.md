...
**Note**: this page describes the Hive Engine NFT market behavior. Do not assume every Hive Engine feature is available on legacy Steem Engine deployments.
...
**Note**: this section only applies to Hive Engine. These features are not available on legacy Steem Engine deployments.
...
For now, the market only supports sell side orders. The ability to place bids will be added later. Also, only Hive accounts can place market orders. Smart contracts that hold tokens cannot currently use the market.
...
- can be called by: Hive account that holds the token(s) to be sold
...
- can be called by: Hive account that originally placed the order(s) to be modified
...
- can be called by: Hive account that originally placed the order(s) to be canceled
...
- parameters:
  ...
  - marketAccount (string): Hive account to receive the market fee percentage of the total sale price. Cannot be the same as the buyer / contract caller. Will be overridden by the officialMarket setting, if such has been set using the [setMarketParams action](#setmarketparams). This account will receive an agent fee instead, if agentCut has been set using the [setMarketParams action](#setmarketparams). So this field can be either the market account or agent account depending on how settings for a particular market are configured, or ignored altogether if officialMarket is set but agentCut is not set.
...
- fields
  - account = the Hive account that created this particular order
...
- fields
  ...
  - account = the buyer's Hive account name
  ...
  - **(optional)** marketAccount = the Hive account that received the market fee for this trade, if there is such a fee
