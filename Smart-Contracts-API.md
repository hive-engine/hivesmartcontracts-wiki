...
actions.removeUser = async (payload) => {
  ...
  if (userId && typeof userId === "string") {
    ...
    if (user) await api.db.remove("users", user);
  }
};
```
...
actions.transferTokensToUser = async (payload) => {
  const { to } = payload;
  await api.executeSmartContractAsOwner("tokens", "transfer", {
    symbol: "TKN",
    quantity: 123.456,
    to,
  });
...
## 4. Check funds sent to a Smart Contract via a Hive transfer operation

When a transaction is sent via a Hive transfer you can access transfer metadata in the payload of the action:

- recipient: the Hive account that received the transfer
- amountHIVEHBD: the amount of HIVE or HBD sent, for example `"10.000 HIVE"` or `"10.000 HBD"`
...
## 6. Hive reference block number

The Hive block number from which the transaction was initiated is available via the following variable:

- api.refHiveBlockNumber: Hive block number

## 7. Hive transaction id
...
- api.random(): returns a random number between 0 and 1 based on the Hive transaction hash, the Hive block hash, and the previous Hive block hash
